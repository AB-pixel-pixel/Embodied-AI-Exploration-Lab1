...
在 Ubuntu 默认终端（GNOME Terminal）中，同时按下ctrl键,alt键和t键，会召唤出一个终端界面：
...
cd Music
```
![1766754879523](image/1.linux/1766754879523.png)

返回上级目录，可以使用：
``` bash
cd ..
```
![1766754896173](image/1.linux/1766754896173.png)

适当使用`tab`可以进行路径提示。
例如，我想进去Music文件夹，但是只记得这个文件夹是Mu开头的，你可以连按两下`tab`:

![1766755015269](image/1.linux/1766755015269.png)
...
- **如在操作过程中，需要使用sudo权限进行某些依赖的安装，请联系助教**
- **本实验手册中除了`rm`,`mv`指令，其余指令都是安全可执行的**
...
**复制粘贴**：
如果想在终端进行复制使用 ` ctrl + shift + c`，如果想使用粘贴可以使用 `ctrl + shift + v`。
在终端中，`ctrl + v`是特殊字符输入模式，在终端中按下会有不显示的字符，不要用它粘贴,需要你按两下回退键进行删除。


**关闭程序**：
-  `ctrl + c`：发送 SIGINT 信号，请求程序终止。
...
##### 2️⃣ 添加执行权限
Linux中文件有"可读、可写、可执行"三种权限。新建的 .sh 文件默认不可执行，需要用 `chmod + x` 添加执行权限。
```bash
