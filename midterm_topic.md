...
| -------- |  -------- |
| **感知**   |  基于 获取激光雷达数据 |
| **控制**   | 基于`/cmd_vel` (geometry_msgs/Twist)   发布速度指令   |
| **可视化**  | RViz   查看雷达数据   |
| **算法参考** | Wall Following / Bug Algorithm / DFS 选择一种或多种实现   |
...
图像处理： 使用  OpenCV (HSV + Contour) 颜色分割与轮廓
...
避障算法，可以使用  VFH / DWA / APF   等 反应式避障策略；
动态检测，可以使用 帧间差分 / 点云对比来识别移动物体 
...
报告应采用 **PDF 格式**，篇幅建议 **2-3 页**（不含代码附录），包含以下内容：
...
| 姓名 | 学号      | 负责模块 | 具体工作内容      | 贡献占比   |
| -- | ------- | ---- | ----------- | ---- | 
| 张三 | 20xx... | 感知模块 | HSV 调参、深度融合 | 30%  |
| 李四 | 20xx... | 规划模块 | A* 算法实现     | 25%  |
| 王五 | 20xx... | 控制模块 | 视觉伺服、速度平滑   | 25%   |
| 赵六 | 20xx... | 集成测试 | 系统调试、视频录制   | 20%   |
