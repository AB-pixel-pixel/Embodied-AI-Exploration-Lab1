...
``` bash python</span></div><code class="hljs code-block-body python">import rospy
...
``` bashpython</span></div><code class="hljs code-block-body python">import rospy
from std_msgs.msg import String
...
``` bashpython</span></div><code class="hljs code-block-body python">from cv_bridge import CvBridge
import cv2
...
``` bashpython</span></div><code class="hljs code-block-body python">from geometry_msgs.msg import Twist
...
``` bashpython</span></div><code class="hljs code-block-body python">from nav_msgs.msg import Odometry
...
``` bashrosrun ros_course_examples run_shape_demo.sh
```
...
``` bashpython</span></div><code class="hljs code-block-body python">def move_square(self):
    side_length = 1.0
    for _ in range(4):
...
``` bashpython</span></div><code class="hljs code-block-body python">import tf.transformations
...
``` bashroslaunch turtlebot3_rviz_launchers view_robot.launch
```

#### 方法3：加载自定义配置文件

``` bashrviz -d /path/to/your_config.rviz
```
...
``` bash</span></div><code class="hljs code-block-body ">┌─────────────────────────────────────────┐
...
5. 配置参数：
   ``` bash</span></div><code class="hljs code-block-body ">✓ Enabled              # 勾选以启用
...
2. 配置参数：
   ``` bash</span></div><code class="hljs code-block-body ">Topic: /odom
...
1. 点击 **"Add"** → 选择 **"TF"**
2. 配置参数：
   ``` bash</span></div><code class="hljs code-block-body ">✓ Show Names          # 显示坐标系名称
   ✓ Show Axes           # 显示坐标轴
   ✓ Show Arrows         # 显示箭头
...
``` bash</span></div><code class="hljs code-block-body ">1. 配置好所有显示项
...
``` bashrviz -d ~/catkin_ws/rviz_configs/my_turtlebot_config.rviz
```

**在 launch 文件中使用**

``` bashxml</span></div><code class="hljs code-block-body xml"><node name="rviz" pkg="rviz" type="rviz" args="-d $(find my_package)/rviz/config.rviz"/>
```
...
``` bash</span></div><code class="hljs code-block-body ">map
 └─ odom
     └─ base_footprint
         └─ base_link
             ├─ camera_link
...
``` bashrostopic echo /scan
```

**查看里程计数据**

``` bashrostopic echo /odom
```

**查看 TF 变换**

``` bash# 查看 map 到 base_link 的变换
rosrun tf tf_echo map base_link
```

**查看话题发布频率**

``` bashrostopic hz /scan
rostopic hz /odom
```
...
``` bash</span></div><code class="hljs code-block-body ">View → Camera Type → Third Person Follower  # 跟随机器人视角
...
``` bashrosrun tf2_tools view_frames.py
evince frames.pdf
```

### 4. TF 变换代码示例

``` bashpython</span></div><code class="hljs code-block-body python">import tf
...
``` bashrosrun ros_course_examples run_game_v1.sh
```
...
``` bashsource src/ros_course_examples/scripts/setup_master.sh
roslaunch ros_course_examples multi_turtlebot_game.launch
```

#### 从机配置（运行控制节点）

``` bash# 假设主机 IP 为 192.168.1.100
